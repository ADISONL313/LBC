<template>
    <div class="certificate-container">
        <logoLBC />
        <h1 class="certificate-title">活动先锋</h1>
        <div class="recipient">敬爱的{{ userDetail.realName }}：</div>
        <div class="message">
            您好！
            我们诚挚的感谢您为留守儿童公益事业做出的卓越贡献！您通过我们的平台，累计参与了公益活动<span class="highlight-span">{{ arCount }}</span>次，您在活动中展现出的热情和奉献精神，深深感染了我们每一个人。我们再次为您献上最真挚的敬意，感谢您为孩子们做出的贡献！本平台授予您活动先锋的称号，期待您在未来的日子里继续为公益事业贡献力量！
        </div>
...
const arCount=ref()
onMounted(async () => {
    get('/AR/getActivityRegistrationCount',{mail:sessionStorage.getItem('mail')}).then((res) => {
        arCount.value = res.data
    })
})
</script>
